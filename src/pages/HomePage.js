...
function HomePage() {
  ...
  //let number = 10
  const [number, setNumber] = useState(); //setNumber เอาใช้เพื่อเปลี่ยนค่า
  ...
  console.log(word);
  const headleClick = () => {
    //  setNumber(number + 1); //ถ้าจะเก็บค่า ให้ไป printนอก setNumber
    ...
    <StyledWrapper>
      ...
      <div>
        <div className='form-item'>
          ...
          <Input id='word-input' placeholder="Basic usage" onChange={(e) => {
      ...
      <Button type='primary' onClick={headleClick} >Add Vocab</Button>

      <Row gutter={[16, 24]}>
        {datalist.map((item, index) => {
          return (
            <Col key={index} xs={24} sm={12} md={8} lg={6}>
              <WordCard  {...item} onDelete={() => {
                handleDelete(index)
              }} />
            </Col>
          )
